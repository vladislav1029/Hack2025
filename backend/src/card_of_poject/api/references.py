...
async def update_service(
    ...
    if not existing:
        raise ResourceNotFoundError()
    if new_service.name != existing.name and await repo_service.get_by_name(new_service.name):
        raise ResourceAlreadyExistsError()
    return await repo_service.update(service_id, new_service.model_dump(exclude_unset=True))
...
# === PAYMENT TYPES ===
@router.post("/payment-types", response_model=PaymentTypeResponse)
...
@router.get("/payment-types/{payment_id}", response_model=PaymentTypeResponse)
...
@router.put("/payment-types/{payment_id}", response_model=PaymentTypeResponse)
async def update_payment_type(
    ...
    if not existing:
        raise ResourceNotFoundError()
    if new_payment.name != existing.name and await repo_payment.get_by_name(new_payment.name):
        raise ResourceAlreadyExistsError()
    return await repo_payment.update(payment_id, new_payment.model_dump(exclude_unset=True))
...
# === BUSINESS SEGMENTS ===
@router.post("/business-segments", response_model=BusinessSegmentResponse)
...
@router.get("/business-segments/{segment_id}", response_model=BusinessSegmentResponse)
...
@router.put("/business-segments/{segment_id}", response_model=BusinessSegmentResponse)
async def update_business_segment(
    ...
    if not existing:
        raise ResourceNotFoundError()
    if new_segment.name != existing.name and await repo_segment.get_by_name(new_segment.name):
        raise ResourceAlreadyExistsError()
    return await repo_segment.update(segment_id, new_segment.model_dump(exclude_unset=True))


@router.delete("/business-segments/{segment_id}")
...
@router.get("/business-segments", response_model=List[BusinessSegmentResponse])
async def list_business_segments(repo_segment: DepBusinessSegmentRep):
    return await repo_segment.list()
# === EVALUATION TYPES ===
@router.post("/evaluation-types", response_model=EvaluationTypeResponse)
...
@router.get("/evaluation-types/{evaluation_id}", response_model=EvaluationTypeResponse)
async def get_evaluation_type(evaluation_id: PyUUID, repo_evaluation: DepEvaluationTypeRep):
...
@router.put("/evaluation-types/{evaluation_id}", response_model=EvaluationTypeResponse)
async def update_evaluation_type(
    ...
    if not existing:
        raise ResourceNotFoundError()
    if new_evaluation.name != existing.name and await repo_evaluation.get_by_name(new_evaluation.name):
        raise ResourceAlreadyExistsError()
    return await repo_evaluation.update(evaluation_id, new_evaluation.model_dump(exclude_unset=True))


@router.delete("/evaluation-types/{evaluation_id}")
...
# === COST TYPES ===
@router.post("/cost-types", response_model=CostTypeResponse)
...
@router.get("/cost-types/{cost_id}", response_model=CostTypeResponse)
...
@router.put("/cost-types/{cost_id}", response_model=CostTypeResponse)
...
@router.delete("/cost-types/{cost_id}")
...
@router.get("/cost-types", response_model=List[CostTypeResponse])
async def list_cost_types(repo_cost: DepCostTypeRep):
    return await repo_cost.list()
# === REVENUE STATUSES ===
@router.post("/revenue-statuses", response_model=RevenueStatusResponse)
...
@router.get("/revenue-statuses/{status_id}", response_model=RevenueStatusResponse)
...
@router.put("/revenue-statuses/{status_id}", response_model=RevenueStatusResponse)
async def update_revenue_status(
    ...
    if not existing:
        raise ResourceNotFoundError()
    if new_status.name != existing.name and await repo_status.get_by_name(new_status.name):
        raise ResourceAlreadyExistsError()
    return await repo_status.update(status_id, new_status.model_dump(exclude_unset=True))


@router.delete("/revenue-statuses/{status_id}")
...
@router.get("/revenue-statuses", response_model=List[RevenueStatusResponse])
async def list_revenue_statuses(repo_status: DepRevenueStatusRep):
    return await repo_status.list()
# Пример для CostStatus

# === COST STATUSES ===
@router.post("/cost-statuses", response_model=CostStatusResponse)
...
@router.get("/cost-statuses/{status_id}", response_model=CostStatusResponse)
...
@router.put("/cost-statuses/{status_id}", response_model=CostStatusResponse)
...
@router.delete("/cost-statuses/{status_id}")
...
@router.get("/cost-statuses", response_model=List[CostStatusResponse])

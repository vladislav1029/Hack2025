...
from src.card_of_poject.repository import (FinancialPeriodRepository, StageRepository,
    CommentRepository,
    ServiceRepository,
    CostTypeRepository,
...
async def get_financial_period_repository(session: AsyncSessionDep) -> FinancialPeriodRepository:
    return FinancialPeriodRepository(session)

async def get_project_history_repository(session: AsyncSessionDep) -> ProjectHistoryRepository:
...
async def get_revenue_status_repository(session: AsyncSessionDep) -> RevenueStatusRepository:
    return RevenueStatusRepository(session)

async def get_payment_type_repository(session: AsyncSessionDep) -> PaymentTypeRepository:
    return PaymentTypeRepository(session)

async def get_evaluation_type_repository(session: AsyncSessionDep) -> EvaluationTypeRepository:
    return EvaluationTypeRepository(session)

async def get_business_segment_repository(session: AsyncSessionDep) -> BusinessSegmentRepository:
    return BusinessSegmentRepository(session)


# === Зависимости для DI ===

DepFinancialPeriodRep = Annotated[FinancialPeriodRepository, Depends(get_financial_period_repository)]
DepProjectHistoryRep = Annotated[ProjectHistoryRepository, Depends(get_project_history_repository)]
...
DepRevenueStatusRep = Annotated[RevenueStatusRepository, Depends(get_revenue_status_repository)]
DepPaymentTypeRep = Annotated[PaymentTypeRepository, Depends(get_payment_type_repository)]
DepEvaluationTypeRep = Annotated[EvaluationTypeRepository, Depends(get_evaluation_type_repository)]
DepBusinessSegmentRep = Annotated[BusinessSegmentRepository, Depends(get_business_segment_repository)]
